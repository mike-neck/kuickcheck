...
internal class ShortGenerator(val min: Short = Short.MIN_VALUE, val max: Short = Short.MAX_VALUE) : Generator<Short> {

    init {
        if (max < min) throw IllegalArgumentException("Max should be larger than min. [max: $max, min: $min]")
    }

    override fun invoke(): Short = RandomSource.nextShort(min, max)
}
